...
public class BarcodeController extends MolgenisPlugin 
{
	...
	public String calculate(HttpServletRequest request, Model model) throws HandleRequestDelegationException, Exception 
	{
		...
		{
			if (null != this.currentType && this.currentNumber != null && 0 < this.currentNumber && this.barcodeTypes.contains(this.currentType)) 
			{
				//TODO: replace hard coded exception with a function which works for all situations 
	...
	public void calcBarcodeSet() 
	{
		...
		if (barcodeTypeList.size() < this.currentNumber) // more samples than barcodes
		{ 
			return;
		} 
		else 
		{
			...
			if (null != defaults) // default option was given
			{ 
				this.optimalCombinations = defaults;
			} 
			else // we have to do the work ourselves
			{ 
	...
	private int[][] getDistance(List<Tuple> barcodeTypeList) 
	{
		...
		for (int i = 0; i < n_barcodes - 1; i++) 
		{
			for (int j = i + 1; j < n_barcodes; j++) 
			{
				...
				{
					if (bc_i.charAt(k) != bc_j.charAt(k))
					{
						dist[i][j]++;
					}
				}
	...
	/**
	 * 
	 * @return three nested list. From inside to outside the lists are: -
	 *         barcode type, id, barcode - a solution - list of solutions
	 */
	public List<List<List<String>>> getOptimalCombinations() 
	{
		List<List<List<String>>> resultList = new ArrayList<List<List<String>>>();
		for (List<Tuple> combi : this.optimalCombinations) // for each solution
		{ 
			List<List<String>> solution = new ArrayList<List<String>>();
			for (Tuple t : combi) // for each barcode in the solution 
			{
	...
	class BarCodeRequest 
	{
		...
		public BarCodeRequest(String barCodeType) 
		{
